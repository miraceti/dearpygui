...
class Hello_World:
	def __init__(self, name = "",xpos=50,ypos=50):
		self.winID = "Hello_World_win_" + str(dpg.generate_uuid())
		with dpg.window(tag=self.winID, pos=(xpos,ypos), width=-1, label="Hello World window " + str(name)) :
			dpg.add_text("Hello, World!")
			dpg.add_button(label="Kill window",width=-1, callback=lambda:dpg.delete_item(self.winID))

hello_world_win = Hello_World(1, 100,100)
hello_world_win2 = Hello_World(2,200, 200)

dpg.setup_dearpygui()
dpg.set_primary_window("main_win", True)#rend cette fenetre fullscreen
dpg.show_viewport()
